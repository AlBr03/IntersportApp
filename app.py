...
class EmailWorker(QRunnable):
    """Run a callable in a background thread and emit finished/error."""
...
def _run_reminders(self, auto: bool):
    """Start reminder verzending in achtergrondthreads zodat de UI niet blokkeert."""
    if auto:
    ...
    for row in candidates:
        ...
        def _ok(_result=None, rid=row_id):
            try:
                ...
                state["failed"] += 1
                print(f"[{'AUTO' if state['auto'] else 'MANUAL'} REMINDER] mark sent failed for id={rid}: {e}")
            finally:
                state["pending"] -= 1
                if state["pending"] <= 0:
                    _finalize()

        def _err(msg: str, rid=row_id):
            state["failed"] += 1
            print(f"[{'AUTO' if state['auto'] else 'MANUAL'} REMINDER] failed for id={rid}: {msg}")
            state["pending"] -= 1
            if state["pending"] <= 0:
                _finalize()
