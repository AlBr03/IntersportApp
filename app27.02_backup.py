...
def get_request_by_id(row_id: int):
    with connect() as conn:
        return conn.execute("SELECT * FROM requests WHERE id=?", (row_id,)).fetchone()
...
class MainWindow(QWidget):
    ...
    def create_settings_tab(self):
        ...
        credit = QLabel("© 2026 Alexander Brinkman. Alle rechten voorbehouden.")
    ...
    def _get_reminder_candidates(self):
        cutoff_open = datetime.now() - timedelta(days=REMINDER_AFTER_DAYS)
        repeat_cutoff = datetime.now() - timedelta(days=REMINDER_REPEAT_DAYS)

        with connect() as conn:
            rows = conn.execute("SELECT * FROM requests WHERE afgerond=0").fetchall()
        ...
        for row in rows:
            ...
            if not created_at:
                continue
            if created_at > cutoff_open:
                continue

            last_sent = _parse_dt(row[13]) if len(row) > 13 else None
            if last_sent is None or last_sent <= repeat_cutoff:
    ...
    def _send_reminder_email_for_row(self, request_row):
        ...
        klant = request_row[1]
        opmerking = request_row[4]
        verkoper = request_row[2]
        created_at = request_row[12]
        row_id = request_row[0]
    ...
    def refresh_table(self):
        ...
        rows_open = [r for r in self.rows if r[11] == 0]
        rows_done = [r for r in self.rows if r[11] == 1]
    ...
    def mark_complete(self, row_id):
        row_data = [r for r in self.rows if r[0] == row_id][0]
    ...
    def send_auto_message_to_customer(self, row_id: int):
        ...
        contact_value = (row[3] or "").strip()  # email/phone stored in 'email' column
